...
function adstm_updparam() {
    ...
    return 'https://sr01.alidropship.com/themes/update?special_theme_update=davinci2';
}
...
function adstm_theme_api_call( $def, $action, $args ) {
    ...
    if( is_wp_error( $request ) ) {
        ...
        $res = new WP_Error('themes_api_failed',
            '<p>'.__('An Unexpected HTTP Error occurred during the API request.', 'dav2').'</p><p><a href="?" onclick="document.location.reload(); return false;">'.__('Try again', 'dav2').'</a></p>', $request->get_error_message());
    } else {

        $res = unserialize( $request['body'] );

        if( $res === false )
            $res = new WP_Error( 'themes_api_failed', __( 'An unknown error occurred', 'dav2' ), $request['body'] );
    }
