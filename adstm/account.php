...
function verify_username_password( $user, $username, $password ) {
    ...
    if( $username == '' && $referrer ) {
        ...
        $error->add( 'empty_username', __( '<strong>ERROR</strong>: The username field is empty.', 'dav2' ) );
        $result = true;
    }
    ...
    if( ! empty( $username ) && adstm_check_username( $username ) ) {
        ...
        $error->add(
            'invalid_username',
            __( '<strong>ERROR</strong>: Invalid username.', 'dav2' ) .
			' <a href="' . wp_lostpassword_url() . '">' . __( 'Lost your password?', 'dav2' ) . '</a>'
        );
    }
    ...
    if( $password == '' && $referrer) {
        ...
        $error->add( 'empty_password', __( '<strong>ERROR</strong>: The password field is empty.', 'dav2' ) );
        $result = true;
    } else {
        ...
        if( username_exists( $username ) || email_exists( $username ) ) {
            ...
            if( ! wp_check_password( $password, $user->data->user_pass, $user->ID ) ) {
                ...
                $error->add(
                    'incorrect_passsword',
                    sprintf(
                        __( '<strong>ERROR</strong>: The password you entered for the username %s is incorrect.', 'dav2' ),
            ...
            if( ! empty( $user->data->activationKey ) ) {
                $error->add( 'invalid_username', __( '<strong>ERROR</strong>: Account is not activated.', 'dav2' ) );
...
function add_login_errors( $error ) {
    ...
    $errors = [
        'empty'              => __( 'Invalid username', 'dav2' ),
        'not_found_username' => __( 'Username not found', 'dav2' ),
        'not_found_email'    => __( 'Email not found', 'dav2' ),
        'notactivate'        => __( 'Your account is not activated', 'dav2' )
    ];
...
function add_password_errors( $error ) {
    ...
    $errors = [
        'empty'   => __( 'Invalid password', 'dav2' ),
        'invalid' => __( 'Wrong password', 'dav2' )
    ];
